...
(function () {
    ...
    function initEvolvingCursor() {
        ...
        const MODE_LABELS = { life: 'Cursor: Life', evolve: 'Cursor: Evolve', off: 'Cursor: Off' };
        ...
        let cursorMode = readStoredMode();
        let evolutionEnabled = cursorMode === 'evolve';

        function applyMode(mode, persist) {
            cursorMode = mode;
            evolutionEnabled = mode === 'evolve';
            document.body.dataset.cursorMode = mode;
            canvas.style.display = mode === 'life' ? 'none' : '';
            if (mode === 'life') ctx.clearRect(0, 0, width, height);
            if (!evolutionEnabled) {
            ...
            if (persist) {
                try { localStorage.setItem('cursor-mode', mode); } catch (err) { /* ignore */ }
            }
            updateToggleButton();
        ...
        function updateToggleButton() {
            ...
            toggleButton.textContent = MODE_LABELS[cursorMode];
            toggleButton.setAttribute('aria-pressed', cursorMode === 'off' ? 'false' : 'true');
        }
        applyMode(cursorMode, false);
        if (toggleButton) {
            toggleButton.addEventListener('click', () => {
                const nextMode = MODES[(MODES.indexOf(cursorMode) + 1) % MODES.length];
